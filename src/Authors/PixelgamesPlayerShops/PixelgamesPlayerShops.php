...
namespace Authors\PixelgamesPlayerShops;
...
//TODO: y statt "y" in Shops.yml fixen
//TODO: Limits setzen (Preis, Menge, Menge der Shops pro Spieler usw.)
//TODO: Shops nur auf Schildern erlauben
//TODO: Fehlende Funktionen hinzufügen (Remove, List)
//TODO: evtl. bestimmte Rechte, wer einen Shop entfernen darf
//TODO: Fehlende Messages hinzufügen

class PixelgamesPlayerShops extends PluginBase implements Listener{

	private $placeQueue, $shop, $shopText, $lang, $tap, $queue, $items;

	/**
	 * @var \Authors\PixelgamesItemCloud\MainClass
	 */
	...
	public function onEnable(){
		...
		if(!class_exists("\\Authors\\PixelgamesItemCloud\\MainClass", false)){
			$this->getLogger()->error("[DEPENDENCY] Um PixelgamesPlayerShops nutzen zu können, muss PixelgamesItemCloud installiert sein!");
			return;
		}

		if(!class_exists("\\onebone\\economyapi\\EconomyAPI", false)){
			$this->getLogger()->error("[DEPENDENCY] Um PixelgamesPlayerShops nutzen zu können, muss EconomyAPI installiert sein!");
			return;
		}
		...
		if(!$this->lang->exists("added-queue")){
			...
			$langArr["added-queue"] = "§e[PGPlayerShops] Berühre ein am besten ein Schild (oder einen Block), auf dem der PlayerShop erstellt werden soll";
		...
		$this->itemcloud = \Authors\PixelgamesItemCloud\MainClass::getInstance();
				...
				switch(strtolower(array_shift($params))){
					...
					case "c":
						...
						if(!$sender instanceof Player){
							$sender->sendMessage(TextFormat::RED . "[PGPlayerShops] Dieser Befehl muss ingame ausgeführt werden");
							return true;
						}

						if(!$sender->hasPermission("pgplayershops.shop.create")){
							$sender->sendMessage($this->getMessage("no-permission-create-shop"));
							return true;
						}

						if(in_array(strtolower($sender->getLevel()->getName()), $this->getConfig()->get("disallowed-worlds", []))){
						...
						if(trim($itemstring) === "" or trim($amount) === "" or trim($cost) === "" or !is_numeric($amount) or !is_numeric($cost)){
							$sender->sendMessage("§c[PGPlayerShops] Benutzung: /pshop create <ItemID[:Meta]> <Menge> <Verkaufspreis>");
							return true;
						}
	...
	public function onBlockBreak(BlockBreakEvent $event){
		...
		if(isset($this->shop[$loc])){
			...
			if($shop["owner"] == $player->getName()){
				...
				if($player->hasPermission("pgplayershops.shop.destroy.others")){
	...
	public function onBlockTouch(PlayerInteractEvent $event){
		...
		if(isset($this->shop[$loc])){
			if($player->hasPermission("pgplayershops.shop.buy")){
				$shop = $this->shop[$loc];
				...
				if(($cloud = $this->itemcloud->getCloudForPlayer($shop["owner"])) instanceof \Authors\PixelgamesItemCloud\ItemCloud){
					if($shop["amount"] > $cloud->getCount($shop["item"], $shop["meta"])){
						...
						if($player->getInventory()->canAddItem(($item = ItemFactory::get($shop["item"], $shop["meta"], $shop["amount"]))) === false){
							...
							if($api->myMoney($player) > $shop["price"]){
								...
								$api->reduceMoney($player, $shop["price"], true, "PixelgamesPlayerShops");
								...
								$api->addMoney($shop["owner"], $shop["price"], true, "PixelgamesPlayerShops");
	...
	public function getMessage($key, $val = ["%1", "%2", "%3"]){
		...
		return "§4[PGPlayerShops] Fehler: Es gibt keine Message, die \"$key\" genannt ist";
	}
